...
class BitmexClient:
    def __init__(self, public_key: str, secret_key: str, testnet: bool):
        ...
        # for subscribing over websocket
        self._ws_id = 1
        self.ws = None
        ...
        logging.info("Binance Futures Client successfully initialized")
    ...
    def _make_request(self, method: str, endpoint: str, data: typing.Dict):
        ...
        if method == "GET":
            try:
                ...
                logging.error("Connection error while making %s to %s: %s", method, endpoint, e)
                return None
        elif method == "POST":
            try:
        ...
        if response.status_code == 200:
            ...
            logging.error("Error while making request %s request to %s: %s (error code %s)",
    ...
    def get_contracts(self) -> typing.Dict[str, Contract]:
        ...
        if instruments is not None:
            for instrument in instruments:
                contracts[instrument['symbol']] = Contract(instrument, "bitmex")
    ...
    def get_balances(self) -> typing.Dict[str, Balance]:
        ...
        if margin_data is not None:
            for asset in margin_data:
                balances[asset['currency']] = Balance(asset, "bitmex")
    ...
    def get_historical_candles(self, contract: Contract, timeframe: str) -> typing.List[Candle]:
        ...
        if raw_candles is not None:
            for candle in raw_candles:
                candles.append(Candle(candle, "bitmex"))

        return candles

    def place_order(self, contract: Contract, order_type: str, quantity: int, side: str, price: None, tif: str) -> OrderStatus:
        data = dict()
        data['symbol'] = contract.symbol
    ...
    def get_order_status(self, order_id: str, contract: Contract):
        data = dict()
        data['symbol'] = contract.symbol
        data['reverse'] = True # newest first
